...
namespace LoginServer.Utility
{
    public class EncDec
    {
        ...
        public void Load_Hash1()
        {
            int i = 0;
            foreach (var b in HashOne)
            {
                Debugger.Log(0, "LOG", $"{i}: {b.ToString("X")}\n");
                i++;
            }
            Debugger.Log(0, "LOG", "HASH 1 LOADED!\n\n");
        }

        public void Load_Hash2()
        {
            int i = 0;
            foreach (var b in HashTwo)
            {
                Debugger.Log(0, "LOG", $"{i}: {b.ToString("X")}\n");
                i++;
            }
            Debugger.Log(0, "LOG", "HASH 2 LOADED!\n\n");
        }
    }
}
